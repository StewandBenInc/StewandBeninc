    ...
    if (title !== "null") {
        ...
        for (const heading in realData) {
            ...
            for (const subheading in realData[heading]["subheadings"]) {
                document.getElementById(`${heading}`).innerHTML += `<h3>Part: ${realData[heading]["subheadings"][subheading].name}</h3>`;
                for (const item in realData[heading]["subheadings"][subheading]["items"]) {
                    document.getElementById(`${heading}`).innerHTML += `<li>Step: ${realData[heading]["subheadings"][subheading]["items"][item]}</li>`;
                }
